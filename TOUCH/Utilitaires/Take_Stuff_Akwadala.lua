...
local function launchExchangeAndTakeStuff()
    AccountBank = LoadAndConnectBotBanque("C:\\Users\\Administrator\\Downloads\\Script_Bot_Dofus\\TOUCH\\Utilitaires\\Give_Stuff_Akwadala.lua")
    ...
    for _, item in ipairs(TableCraft) do
        if AccountBank.inventory:itemCount(item.Id) == 0 then
            ...
            if AccountBank.character:kamas() * 0.9 < (TotalCost + 80000) then
                global:printSuccess("Le bot bank n'a pas assez de kamas, on attend 2 heures")
                AccountBank:disconnect()
                global:reconnect(2)
            else
                AccountBank.global:loadAndStart("C:\\Users\\Administrator\\Downloads\\Script_Bot_Dofus\\TOUCH\\MetierCraft\\CraftPannaeauAkwadala.lua")
