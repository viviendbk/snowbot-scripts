...
function fightManagement()
    if fightCharacter:isItMyTurn() then
        ...
        if (fightAction:getCurrentTurn() - 1) % 6 == 0 and fightCharacter:getLevel() > 47 then
			NbPa = fightCharacter:getAP()
            LaunchLapino()
			fightCharacter:forceAP(NbPa - 3)
        elseif (fightAction:getCurrentTurn() - 1) % 3 == 0 then
            NbPa = fightCharacter:getAP()
		...
		if LowEnemies ~= nil and #LowEnemies > 0 then
			for _, entity in ipairs(LowEnemies) do
				if fightCharacter:getAP() > 2 then
					...
					if fightCharacter:getLevel() > 99 and isEnemyStillHere(entity.CellId) and fightCharacter:getAP() == 10 or fightCharacter:getAP() == 7 then
		...
		-- si on a encore beaucoup de pa, on se rapproche au max de l'adversaire
