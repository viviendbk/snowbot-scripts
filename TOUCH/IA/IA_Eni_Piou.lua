...
local function MoveInLineOf(cellId) -- uniquement pour Mot Concussifs
	...
	if (not Contain(listCellId, fightCharacter:getCellId()) and #fightAction:getShortestPath(fightCharacter:getCellId(), cellId, false) > 7) 
...
local function Mot_Stimuant()
	if fightAction:canCastSpellOnCell(fightCharacter:getCellId(), 5803, fightCharacter:getCellId()) == 0 then 
		fightAction:castSpellOnCell(5803, fightCharacter:getCellId())
	end	
end
...
function fightManagement()
    if fightCharacter:isItMyTurn() then
		...
		if random < 0.05 then
			global:delay(math.random(7000, 8500))
		elseif random < 0.2 then
			global:delay(math.random(3000, 3500))
		else
			global:delay(math.random(1000, 1500))
		end

        if (fightAction:getCurrentTurn() - 1) % 3 == 0 and fightCharacter:getLevel() > 20 then
		...
		if enemies ~= nil and #enemies > 0 then
			for _, entity in ipairs(enemies) do
				if fightCharacter:getAP() > 2 and fightCharacter:getLevel() > 25 then
					MoveTo(entity.CellId, 4, true)
