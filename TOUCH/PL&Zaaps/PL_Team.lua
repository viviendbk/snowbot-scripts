...
if not global:remember("LEVEL_MINI") then
    global:addInMemory("LEVEL_MINI", character:level())
else
    global:editInMemory("LEVEL_MINI", character:level())
end
...
function _ExchangeRequestedTradeMessage(message)
    ...
    -- if global:isBoss() and ((global:remember("LEVEL_MINI") < 40) and job:level(41) > 45) then
...
function move()
    ...
    --- utilitaire
    if global:isBoss() and global:maximumNumberFightsOfDay() then
        local team = global:thisAccountController():getTeamAccounts()
        local random = math.random(120, 180)
        for _, acc in ipairs(team) do
            acc.global:reconnectBis(random)
        end
        global:reconnectBis(random)
    end
    ...
    if global:isBoss() then
        local lvlMini = character:level()
        local team = global:thisAccountController():getTeamAccounts()
        for _, acc in ipairs(team) do
            lvlMini = math.min(lvlMini, acc.character:level())
        end
        for _, acc in ipairs(team) do
            acc.global:editInMemory("LEVEL_MINI", lvlMini)
        end
        global:editInMemory("LEVEL_MINI", lvlMini)
    end
    --- utilitaire

    --- prendre la panneau 70
    if global:isBoss() and global:remember("LEVEL_MINI") > 69 and not AllTeamHavePanneau70() then
        -- si les bots sont en team, qu'ils sont plus que lvl 69 et qu'ils ont pas la panneau 70, on les fait se déco et se reco alone
        local Team = global:thisAccountController():getTeamAccounts()
        for _, acc in ipairs(Team) do
            acc:callScriptFunction("ProcessBank")
        end
        ProcessBank()
        for _, acc in ipairs(Team) do
            acc.global:editAlias(acc.global:thisAccountController():getAlias() .. " Need1", true)
    ...
    if not global:thisAccountController():isItATeam() and character:level() > 69 and inventory:itemCount(2531) == 0 then
    ...
    if global:isBoss() and #global:thisAccountController():getTeamAccounts() < 3 and inventory:itemCount(2531) == 0 and global:remember("LEVEL_MINI") > 69 then
        -- si les bots sont en team, qu'ils sont plus que lvl 69 et qu'ils ont pas la panneau 70, on les fait se déco et se reco alone
        local Team = global:thisAccountController():getTeamAccounts()
        for _, acc in ipairs(Team) do
    ...
    if not global:thisAccountController():isItATeam() and character:level() > 69 then
        global:editAlias(global:thisAccountController():getAlias() .. " Need4", true)
        global:disconnect()
    end
    --- changement de script 

    --- se mettre en team de 2
    if global:remember("LEVEL_MINI") < 70 and not global:thisAccountController():isItATeam() then
        global:editAlias(global:thisAccountController():getAlias() .. " Need2", true)
        global:disconnect()
    end
    --- se mettre en team de 2
    ...
    if global:remember("LEVEL_MINI") < 70 and global:thisAccountController():isItATeam() then
        MIN_MONSTERS = 1
        return treatMaps(TrajetBouftou)   
    elseif global:remember("LEVEL_MINI") >= 70 and global:thisAccountController():isItATeam() then
        local Team = global:thisAccountController():getTeamAccounts()
        for _, acc in ipairs(Team) do
