...
function DeplacementProche()
    ...
    local path = fightAction:getShortestPath(myCellId, enemy, false)
    ...
    elseif not IsHandToHandEnemy() then
        ...
        table.sort(cellProches, function(a, b)
            ...
            local pathA = #fightAction:getShortestPath(myCellId, a, false)
            local pathB = #fightAction:getShortestPath(myCellId, b, false)
        ...
        for _, element in ipairs(cellProches) do
            local newPath = fightAction:getShortestPath(myCellId, element, false)
...
function Condensation()
	if fightCharacter:getAP() > 2 then
		cellid = BestChoiceForZone(fightCharacter:getCellId(), 12745, WeakerMonsterAdjacent(), 3, true)[1]
...
function Douleur_Cuisante()
	if fightCharacter:getAP() > 2 and fightCharacter:getLevel() > 74 then
...
function Entaille(cellid)
	if fightCharacter:getLevel() > 179 and fightAction:canCastSpellOnCell(fightCharacter:getCellId(), 12751, cellid) == 0 then 
		fightAction:castSpellOnCell(12751, cellid)
        actionFightDelay()
	end
end

function Decimation(cellid)
