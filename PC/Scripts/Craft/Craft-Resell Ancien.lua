...
function move()
    ...
    if ScriptStarting then
        ...
        elseif not hdvRessourceChecked then
            ...
            if isXDaysLater(jsonPrice[1].Date, 2) then
                global:printSuccess("Le prix des ressources ne sont pas à jour, on attends un peu")
        ...
        elseif not hdvEquipChecked then
            ...
            global:printSuccess("Vente terminée!")
            ...
            for _, item in ipairs(TableItem) do
                ...
                and podsAvailable > item.PodsNeededToCraft * 2 and (kamasOwned - item.TotalCost * item.NbToCraft * 1.1) > 100000 then
                    ...
                    if item.ListIdCraft then
                        ...
                        global:printSuccess("[Item] : " .. inventory:itemNameId(item.Id) .. ", [Coût de craft] : " .. item.TotalCost .. ", [Revente] : " .. item.PriceHdv .. ", [Renta Mini] : " .. (item.PriceHdv / item.TotalCost) .. "%")
                        ...
                        elseif item.Type == "Bouclier" or item.Type == "Trophet" or item.Type == "Idole" or item.Type == "Prysmaradite" then
            ...
            global:printMessage("- " .. #CraftFaconneur .. " craft faconneur")
        ...
        elseif not hdvRessourceChecked2 then
            ...
            for i = #CraftCordonier, 1, -1 do
                ...
                if not ((item.PriceHdv / realPrice) > 1.3 and (item.PriceHdv - realPrice) > 10000) then
                    global:printError("L'item " .. inventory:itemNameId(item.Id) .. " n'est plus rentable à faire")
                    table.remove(CraftCordonier, i)
                else
                    table.insert(newTemp, item.Id)
                end
            end
            for i = #CraftFaconneur, 1, -1 do
                local item = CraftFaconneur[i]
                local realPrice = 0
                for _, ressource in ipairs(item.ListIdCraft) do
                    realPrice = realPrice + GetPricesItem(ressource.Id).TrueAveragePrice + ressource.Quantity
                end
    ...
    for _, item in ipairs(CraftFaconneur) do
        if inventory:itemCount(item.Id) < item.NbToCraft then
            if not map:onMap(81788930) then
                ...
                ProcessCraft(CraftFaconneur, 443, 60)
            end
        end
    end
    for _, item in ipairs(CraftCordonier) do
    ...
    elseif not ItemSold then
        local content = inventory:inventoryContent()
        for _, item in ipairs(content) do
            if inventory:itemNameId(item.objectGID) == "Idole" then
                GoSellIdole = true
            end
            for _, element in ipairs(TableItem) do
                if item.objectGID == element.Id then
                    while inventory:itemCount(item.objectGID) > 0 and inventory:itemPosition(item.objectGID) == 63 and IsItem(inventory:itemTypeId(item.objectGID)) 
                    and sale:availableSpace() > 0 and inventory:itemNameId(item.objectGID) ~= "Idole" do
                        global:printMessage("Cout de craft pour " .. inventory:itemCount(element.Id) .. " : " .. element.TotalCost)
                        SellItem(item.objectGID, (element.TotalCost * 2.5) > 100000 and element.TotalCost * 2.5 or 100000)
                    end
                end
            end
        end
    ...
    if not map:onMap(212601350) and GoSellIdole then
        return TreatMaps(goToHdvRessources)
    elseif GoSellIdole then
        HdvSell()
        local content = inventory:inventoryContent()
        for _, item in ipairs(content) do
            while inventory:itemCount(item.objectGID) > 0 and inventory:itemPosition(item.objectGID) == 63 and IsItem(inventory:itemTypeId(item.objectGID)) and sale:availableSpace() > 0 and inventory:getTypeName(item.objectGID) == "Idole" do
                SellItem(item.objectGID)
            end
        end
        GoSellIdole = false
