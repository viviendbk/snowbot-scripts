    ...
    if ScriptStarting then
        ...
        if #TableItemToFM > 0 and not memoryChecked then
            ...
            for _, item in ipairs(TableItemToFM) do
                ...
                if item.Type == "Chapeau" or item.Type == "Cape" or item.Type == "Sac à dos" then
                    table.insert(CraftTailleur, {Id = item.Id, TotalCost = item.TotalCost, Type = item.Type, PodsNeededToCraft = item.PodsNeededToCraft, ListIdCraft = getIngredients(item.Id), NbToCraft = item.NbToCraft, InfoFm = item.InfoFm, RunesCost = item.RunesCost, PriceFM = item.PriceFM})
                elseif item.Type == "Ceinture" or item.Type == "Bottes" then
                    table.insert(CraftCordonier, {Id = item.Id, TotalCost = item.TotalCost, Type = item.Type, PodsNeededToCraft = item.PodsNeededToCraft, ListIdCraft = getIngredients(item.Id), NbToCraft = item.NbToCraft, InfoFm = item.InfoFm, RunesCost = item.RunesCost, PriceFM = item.PriceFM})
                elseif item.Type == "Anneau" or item.Type == "Amulette" then
                    table.insert(CraftBijoutier, {Id = item.Id, TotalCost = item.TotalCost, Type = item.Type, PodsNeededToCraft = item.PodsNeededToCraft, ListIdCraft = getIngredients(item.Id), NbToCraft = item.NbToCraft, InfoFm = item.InfoFm, RunesCost = item.RunesCost, PriceFM = item.PriceFM})
                elseif item.Type == "Épée" or item.Type == "Hache" or item.Type == "Marteau" or item.Type == "Dague" or item.Type == "Pelle" then
                    table.insert(CraftForgeron, {Id = item.Id, TotalCost = item.TotalCost, Type = item.Type, PodsNeededToCraft = item.PodsNeededToCraft, ListIdCraft = getIngredients(item.Id), NbToCraft = item.NbToCraft, InfoFm = item.InfoFm, RunesCost = item.RunesCost, PriceFM = item.PriceFM})
                elseif item.Type == "Bâton" or item.Type == "Baguette" or item.Type == "Arc" then
                    table.insert(CraftSculpteur, {Id = item.Id, TotalCost = item.TotalCost, Type = item.Type, PodsNeededToCraft = item.PodsNeededToCraft, ListIdCraft = getIngredients(item.Id), NbToCraft = item.NbToCraft, InfoFm = item.InfoFm, RunesCost = item.RunesCost, PriceFM = item.PriceFM})
                end
            end
        end
        memoryChecked = true
        ...
        elseif not bankChecked then
            ...
            for _, item in ipairs(bankContent) do
                if IsItem(inventory:itemTypeId(item)) then
                    dicoItems[tostring(item)] = not dicoItems[tostring(item)] and exchange:storageItemQuantity(item) or dicoItems[tostring(item)] + exchange:storageItemQuantity(item)
                    if exchange:storageItemQuantity(item) > 1 then
                        for i = 1, exchange:storageItemQuantity(item) do
                            if exchange:storageItemQuantity(item) > 0 and inventory:itemCount(item) == 0 then
        ...
        elseif goBreak then
            ...
            for k, v in pairs(dicoItems) do
                ...
                if v > 1 and inventory:itemCount(tonumber(k)) > 1 and inventory:itemPosition(tonumber(k)) == 63 then
           ...
           local jsonPrice = openFile(global:getCurrentScriptDirectory() .. "\\".. character:server() .. "\\PriceItems.json")
            ...
            for _, item in ipairs(TableItem) do
                ...
                if not jsonPrice[1].Prices[tostring(item.Id)] then
                    jsonPrice[1].Prices[tostring(item.Id)] = GetPricesItem(item.Id).Price1
                end

                item.PriceHdv = jsonPrice[1].Prices[tostring(item.Id)]
            end
            ...
            if not FirstSellDone then
                ...
                for _, item in ipairs(content) do
                    for _, item2 in ipairs(TableItem) do
                        if item.objectGID == item2.Id and item2.TotalCost * 1.2 < item2.PriceHdv then
                            ...
                            and sale:availableSpace() > 0 and cpt < 2 and ItemSatisfyConditionsById(item.objectGID) and inventory:getLevel(item.objectGID) > 119 do
                                SellItem(item.objectGID)
                                cpt = cpt + 1
                            end
                        end
                    end
            ...
            for _, item in ipairs(TableItem) do
                ...
                and not IsBlackList(item.Id) then
                    ...
                    if (item.PriceFM > item.TotalCost * 1.3) and 
                    ...
                    or ((item.PriceFM - item.TotalCost) > 75000)) then
                        ...
                        if item.ListIdCraft and ItemHasTwoOrMoreCarac(item.Id) then
                            ...
                            if (item.Type == "Chapeau" or item.Type == "Cape" or item.Type == "Sac à dos") and #CraftTailleur < (job:level(GetJobIdByType(item.Type)) > 160 and 10 or 6) then
                                table.insert(TableItemToFM, item)
                                table.insert(CraftTailleur, item)
                            elseif (item.Type == "Ceinture" or item.Type == "Bottes") and #CraftCordonier < (job:level(GetJobIdByType(item.Type)) > 160 and 10 or 6) then
                                table.insert(TableItemToFM, item)
                                table.insert(CraftCordonier, item)
                            elseif (item.Type == "Anneau" or item.Type == "Amulette") and #CraftBijoutier < (job:level(GetJobIdByType(item.Type)) > 160 and 10 or 6) then
                                table.insert(TableItemToFM, item)
                                table.insert(CraftBijoutier, item)
                            elseif (item.Type == "Épée" or item.Type == "Hache" or item.Type == "Marteau" or item.Type == "Dague" or item.Type == "Pelle") and #CraftForgeron < (job:level(GetJobIdByType(item.Type)) > 160 and 10 or 6) then
                                table.insert(TableItemToFM, item)
                                table.insert(CraftForgeron, item)
                            elseif (item.Type == "Bâton" or item.Type == "Baguette" or item.Type == "Arc") and #CraftSculpteur < (job:level(GetJobIdByType(item.Type)) > 160 and 10 or 6) then
                                table.insert(TableItemToFM, item)
            ...
            global:printMessage("- " .. #CraftForgeron .. " craft forgeron")
            global:printMessage("- " .. #CraftSculpteur .. " craft sculpteur")
            steep = 0 global:leaveDialog()     
        ...
        elseif not hdvRessourceChecked2 then
            ...
            for i = #CraftBijoutier, 1, -1 do
                ...
                if not ((item.PriceHdv / realPrice) > 1.3 and (item.PriceHdv - realPrice) > 10000) then
                    ...
                    for i = #TableItemToFM, 1, -1 do
                        if TableItemToFM[i].Id == item.Id then
            ...
            for i = #CraftCordonier, 1, -1 do
                local item = CraftCordonier[i]
                local realPrice = 0
                for _, ressource in ipairs(item.ListIdCraft) do
                    realPrice = realPrice + GetPricesItem(ressource.Id).TrueAveragePrice + ressource.Quantity
                end
            ...
            for i = #CraftForgeron, 1, -1 do
                local item = CraftForgeron[i]
                local realPrice = 0
                for _, ressource in ipairs(item.ListIdCraft) do
                    realPrice = realPrice + GetPricesItem(ressource.Id).TrueAveragePrice + ressource.Quantity
                end
            ...
            for i = #CraftSculpteur, 1, -1 do
                local item = CraftSculpteur[i]
                local realPrice = 0
                for _, ressource in ipairs(item.ListIdCraft) do
                    realPrice = realPrice + GetPricesItem(ressource.Id).TrueAveragePrice + ressource.Quantity
                end
            ...
            for i = #CraftTailleur, 1, -1 do
                local item = CraftTailleur[i]
                local realPrice = 0
                for _, ressource in ipairs(item.ListIdCraft) do
                    realPrice = realPrice + GetPricesItem(ressource.Id).TrueAveragePrice + ressource.Quantity
                end
            ...
            for i, item2 in ipairs(TableItemToFM) do
                if item.objectGID == item2.Id then
                    ...
                    if not over then
                        ...
                        for _, rune in ipairs(RunesTransVita) do
                            if item2.InfoFm.Stats["Vitalite"].Current * PoidsByStat["Vitalite"].PoidsUnite < rune.ConditionPoids then
                    ...
                    and sale:availableSpace() > 0 do
                        ...
                        if item2.RuneTrans then
                            ...
                            SellItem(item.objectGID, math.floor(item2.PriceWithoutTrans + (item2.RuneTrans.Prices.AveragePrice * 3)))
                        else
                            SellItem(item.objectGID, math.floor((item2.TotalCost  * 2 +  item2.RunesCost * 3)))
                        end
                        table.remove(TableItemToFM, i)
                        EditJsonMemory(TableItemToFM)
                    end
