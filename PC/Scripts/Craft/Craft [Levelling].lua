    ...
    elseif not brisageDone then
        ...
        for x = 1, 2 do
            ...
            for _, element in ipairs(content) do
                if IsItem(inventory:itemTypeId(element.objectGID)) and element.position == 63 then
                    table.insert(items, element.objectUID)
                end
            end
            local message = developer:createMessage("ExchangeobjectransfertListFromInvMessage")
            message.ids = items
            developer:sendMessage(message)
