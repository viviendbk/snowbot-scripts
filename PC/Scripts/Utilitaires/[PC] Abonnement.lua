 -- Read in JSON
JSONFile = "C:\\Users\\Administrator\\Downloads\\Script_Bot_Dofus\\PC\\Scripts\\Utilitaires\\JSON.lua"
...
function getRemainingSubscription(inDay, acc)
	...
	global:printSuccess("222")
	global:printSuccess(endDate)
	global:printSuccess(math.floor(endDate / 24))
	global:printSuccess("aaa")

    return inDay and tonumber(math.floor(endDate / 24)) or endDate
end
		...
		if offer_rate ~= nil and offer_amount ~= nil then
			...
			if character:kamas() >= needed_kamas then
				...
				if not global:thisAccountController():getAlias():find("Groupe") then
					if global:thisAccountController():getAlias():find("Mineur") and job:level(24) > 40
					or global:thisAccountController():getAlias():find("Bucheron") and job:level(2) > 40
					or global:thisAccountController():getAlias():find("LvlUp") and character:level() > 100
					or global:thisAccountController():getAlias():find("Combat") then
