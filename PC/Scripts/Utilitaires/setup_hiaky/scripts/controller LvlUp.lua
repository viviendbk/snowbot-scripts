...
local function ExporterComptes()
    ...
    local f = ""
    if #AccountToLoad.Combat > 0 then
        f = io.open("C:\\Users\\Administrator\\Documents\\snowbot-scripts\\PC\\Assets\\ComptesCombat.txt", "w")
    elseif #AccountToLoad.LvlUp > 0 then
        f = io.open("C:\\Users\\Administrator\\Documents\\snowbot-scripts\\PC\\Assets\\ComptesLvlUp.txt", "w")
    elseif #AccountToLoad.Craft > 0 then
        f = io.open("C:\\Users\\Administrator\\Documents\\snowbot-scripts\\PC\\Assets\\ComptesCraft.txt", "w")
    end
        ...
        for i = 1, NB_LVLUP do
            for _, Alias in ipairs(AliasAllAccount) do
                if Alias:find("LvlUp" .. i) and GetServerByAlias(Alias):find(server) then
                    ...
                    lvlupPresents[i] = true
                    break
                else
                    lvlupPresents[i] = false
                end
            end
        end

        for _, canLoadNewAccount in ipairs(lvlupPresents) do
            if not canLoadNewAccount then

                global:printSuccess("On peut créer un nouveau compte LvlUp" .. _ .. " " .. server)
                    ...
                    if IsInTable(SERVERS_MONO, server) and Alias:find("Next") then
                        ...
                        for j, username in ipairs(UsernameNotLoaded) do
                            if i == j then
                                ...
                                snowbotController:assignProxyToAnAccount(username, PROXIES["2"].ips,  PROXIES["2"].port,  PROXIES["2"].username,  PROXIES["2"].password, (TYPE_PROXY ~= "socks5"), true)
...
function move()
    ...
    for _, acc in ipairs(LoadedAccounts) do
        if acc:isTeamLeader() and acc:isAccountFullyConnected() and not acc:isScriptPlaying() then
            local team = acc:getTeamAccounts()
            local ready = true
            for _, acc in ipairs(team) do
                if not acc:isAccountFullyConnected() then
